...
def collect_training_data():
    conn = None
    try:
        logger.info("STEP 1: collecting traffic-rainfall ML training snapshot")
        conn = get_connection()
...
with DAG(
    ...
    description="Persist traffic + rainfall snapshots for ML training",
    schedule="*/5 * * * *",
    start_date=datetime(2026, 4, 21),
    catchup=False,
    max_active_runs=1,
    tags=["ml", "training", "traffic", "rainfall"],
) as dag:
